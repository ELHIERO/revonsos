...
### It also provides convenient targets for running and debugging RevonsOs and using GDB on your host computer.
.DEFAULT_GOAL := all
SHELL := /bin/bash
...
ifeq ($(UNAME),Darwin)
	...
	GRUB_CROSS = "$(HOME)"/revonsos_tools_opt/bin/
...
check-rustc:
## Building RevonsOs requires the 'rust-src' component. If we can't install that, install the required rust toolchain and retry.
...
###################################################################################################
### This section contains targets to actually build RevonsOs components and create an iso file.
###################################################################################################

BUILD_DIR := $(ROOT_DIR)/build
NANO_CORE_BUILD_DIR := $(BUILD_DIR)/nano_core
iso := $(BUILD_DIR)/revonsos-$(ARCH).iso
...
REVONSOS_BUILD_TOML := $(DEPS_DIR)/RevonsOsBuild.toml
REVONSOS_CARGO := $(ROOT_DIR)/tools/revonsos_cargo
REVONSOS_CARGO_BIN := $(REVONSOS_CARGO)/bin/revonsos_cargo
...
## Copy all object files into the main build directory and prepend the kernel or app prefix appropriately. 
	@cargo run --release --manifest-path $(ROOT_DIR)/tools/copy_latest_crate_objects/Cargo.toml -- \
		...
		-e "$(EXTRA_APP_CRATE_NAMES) librevonsos"
...
## and most importantly, a TOML file to describe these and other config variables.
	@rm -rf $(REVONSOS_BUILD_TOML)
	@cp -vf $(CFG_DIR)/$(TARGET).json  $(DEPS_DIR)/
	@mkdir -p $(HOST_DEPS_DIR)
	@cp -f ./target/$(BUILD_MODE)/deps/*  $(HOST_DEPS_DIR)/
	@echo -e 'target = "$(TARGET)"' >> $(REVONSOS_BUILD_TOML)
	@echo -e 'sysroot = "./sysroot"' >> $(REVONSOS_BUILD_TOML)
	@echo -e 'rustflags = "$(RUSTFLAGS)"' >> $(REVONSOS_BUILD_TOML)
	@echo -e 'cargoflags = "$(CARGOFLAGS)"' >> $(REVONSOS_BUILD_TOML)
	@echo -e 'host_deps = "./host_deps"' >> $(REVONSOS_BUILD_TOML)
...
cargo: check-rustc 
	...
	@echo -e "\t REVONSOS_CONFIG (before build.rs script): \"$(REVONSOS_CONFIG)\""
...
$(NANO_CORE_BUILD_DIR)/boot/$(ARCH)/%.o: $(NANO_CORE_SRC_DIR)/boot/arch_$(ARCH)/%.asm
    ...
    ## If the user chose to enable VGA text mode by means of setting `REVONSOS_CONFIG`,
...
ifneq (,$(findstring vga_text_mode, $(REVONSOS_CONFIG)))
...
### Target for building tlibc, RevonsOs's libc.
### This should be run after `make iso` has completed.
### It builds a new .iso that includes tlibc, which can be run using `make orun`.
### Currently we can manually load tlibc within RevonsOs for testing purposes by running `ns --load path/to/tlibc_file`.
.PHONY: tlibc
...
# $(MAKE) -C tlibc
	...
	$(MAKE) grub
	@echo -e "\n\033[1;32m The build of tlibc finished successfully and was packaged into the RevonsOs ISO.\033[0m"
	@echo -e "    --> Use 'make orun' to run it now (don't use 'make run', that will overwrite tlibc)"
	@echo -e "    --> In RevonsOs, run 'ns --load /namespaces/_applications/tlibc.o' to load tlibc."
...
c_test:
	...
	$(MAKE) grub
	@echo -e "\n\033[1;32m The build of $(C_TEST_TARGET) finished successfully and was packaged into the RevonsOs ISO.\033[0m"
	@echo -e "    --> Use 'make orun' to run it now (don't use 'make run')"
	@echo -e "    --> In RevonsOs, run 'loadc /namespaces/_executables/$(C_TEST_TARGET)' to load and run the C program."



### Demo/test target for building librevonsos
librevonsos: $(REVONSOS_CARGO_BIN) $(ROOT_DIR)/librevonsos/Cargo.* $(ROOT_DIR)/librevonsos/src/*
	@( \
		cd $(ROOT_DIR)/librevonsos && \
		$(REVONSOS_CARGO_BIN) --input $(DEPS_DIR) build; \
	)



### This target builds the `revonsos_cargo` tool as a dedicated binary.
$(REVONSOS_CARGO_BIN): $(REVONSOS_CARGO)/Cargo.* $(REVONSOS_CARGO)/src/*
	@echo -e "\n=================== Building the revonsos_cargo tool ==================="
	cargo install --force --path=$(REVONSOS_CARGO) --root=$(REVONSOS_CARGO)
...
# ## (This is currently not used in RevonsOs, since we don't run anything in userspace)
...
## It builds everything with the SIMD-enabled x86_64-revonsos-sse target,
## and then builds everything again with the regular x86_64-revonsos target. 
## The "normal" target must come last ('build_sse', THEN the regular 'build') to ensure that the final nano_core_binary is non-SIMD.
simd_personality_sse : export TARGET := x86_64-revonsos
simd_personality_sse : export BUILD_MODE = release
simd_personality_sse : export override REVONSOS_CONFIG += simd_personality simd_personality_sse
...
## It builds everything with the SIMD-enabled x86_64-revonsos-avx target,
## and then builds everything again with the regular x86_64-revonsos target. 
## The "normal" target must come last ('build_avx', THEN the regular 'build') to ensure that the final nano_core_binary is non-SIMD.
simd_personality_avx : export TARGET := x86_64-revonsos
simd_personality_avx : export BUILD_MODE = release
simd_personality_avx : export override REVONSOS_CONFIG += simd_personality simd_personality_avx
...
### build_sse builds the kernel and applications with the x86_64-revonsos-sse target.
### It can serve as part of the simd_personality_sse target.
build_sse : export override TARGET := x86_64-revonsos-sse
...
### build_avx builds the kernel and applications with the x86_64-revonsos-avx target.
### It can serve as part of the simd_personality_avx target.
build_avx : export override TARGET := x86_64-revonsos-avx
...
### build_server is a target that builds RevonsOs into a regular ISO
### and then sets up an HTTP server that provides module object files 
### for a running instance of RevonsOs to download for OTA live updates.
...
RUSTDOC_OUT_FILE := $(RUSTDOC_OUT)/___RevonsOs_Crates___/index.html

## Builds RevonsOs's source-level documentation for all Rust crates except applications.
...
## Now, build the docs for all of RevonsOs's main kernel crates.
	@cargo doc --workspace --no-deps $(addprefix --exclude , $(APP_CRATE_NAMES))
	@rustdoc --output target/doc --crate-name "___RevonsOs_Crates___" $(ROOT_DIR)/kernel/_doc_root.rs
	@rm -rf $(RUSTDOC_OUT)
	@mkdir -p $(RUSTDOC_OUT)
	@cp -rf target/doc/. $(RUSTDOC_OUT)
	@echo -e "\nRevonsOs source docs are now available at: \"$(RUSTDOC_OUT_FILE)\"."
...
### The locations of RevonsOs's book-style documentation.
BOOK_SRC      := $(ROOT_DIR)/book
BOOK_OUT      := $(BUILD_DIR)/book
BOOK_OUT_FILE := $(BOOK_OUT)/html/index.html

### Builds the RevonsOs book-style documentation using `mdbook`.
...
ifneq ($(shell mdbook --version > /dev/null 2>&1 && echo $$?), 0)
	...
	@echo -e "\nThe RevonsOs Book is now available at \"$(BOOK_OUT_FILE)\"."


### Opens the RevonsOs book.
view-book: book
	@echo -e "Opening the RevonsOs book in your browser..."
...
help: 
	...
	@echo -e "   iso:"
	@echo -e "\t The default and most basic target. Builds the full RevonsOs OS and creates a bootable ISO image."

	@echo -e "   run:"
	@echo -e "\t Builds RevonsOs (like the 'iso' target) and runs it using QEMU."
	...
	@echo -e "\t You must run an instance of RevonsOs in QEMU beforehand in a separate terminal."

	@echo -e "   bochs:"
	@echo -e "\t Same as 'make run', but runs RevonsOs in the Bochs emulator instead of QEMU."

	@echo -e "   boot:"
	@echo -e "\t Builds RevonsOs as a bootable .iso and writes it to the specified USB drive."
	...
	@echo -e "   pxe:"
	@echo -e "\t Builds RevonsOs as a bootable .iso and copies it to the tftpboot folder for network booting over PXE."
	...
	@echo -e "\t Builds RevonsOs with a regular personality and a SIMD-enabled personality (either SSE or AVX),"
	@echo -e "\t then runs it just like the 'make run' target."

	@echo -e "   build_server:"
	@echo -e "\t Builds RevonsOs (as with the 'iso' target) and then runs a build server hosted on this machine"
	...
	@echo -e "\t Then, a running instance of RevonsOs version 1 can contact this machine's build_server to update itself to version 2."
...
# @echo -e "\t Enable KVM acceleration (the host computer must support it)."
	...
	@echo -e "   doc:"
	@echo -e "\t Builds RevonsOs documentation from its Rust source code (rustdoc)."
	@echo -e "   view-doc:"
	@echo -e "\t Builds RevonsOs documentation and then opens it in your default browser."
	@echo -e "   book:"
	@echo -e "\t Builds the RevonsOs book using the mdbook Markdown tool."
	@echo -e "   view-book:"
	@echo -e "\t Builds the RevonsOs book and then opens it in your default browser."
...
## We don't yet support SATA in RevonsOs, but this is how to add a SATA drive over the AHCI interface.
...
###################################################################################################
########################## Targets for running and debugging RevonsOs #############################
###################################################################################################
...
### builds and runs RevonsOs in loadable mode, where all crates are dynamically loaded.
loadable : export override RevonsOs_CONFIG += loadable
loadable: run


### builds and runs RevonsOs in QEMU
run: $(iso) 
	qemu-system-x86_64 $(QEMU_FLAGS)


### builds and runs RevonsOs in QEMU, but pauses execution until a GDB instance is connected.
run_pause: $(iso)
	qemu-system-x86_64 $(QEMU_FLAGS) -S
...
### builds and runs RevonsOs in Bochs
bochs : export override REVONSOS_CONFIG += apic_timer_fixed
bochs: $(iso) 
...
boot : export override REVONSOS_CONFIG += mirror_log_to_vga
boot: check-usb $(iso)
ifneq ($(IS_WSL), )
## building on WSL
...
### this builds an ISO and copies it into the revonsos tftpboot folder as described in the REAEDME 
pxe : export override REVONSOS_CONFIG += mirror_log_to_vga
pxe: $(iso)
ifdef $(netdev)
ifdef $(ip)
	...
	@sudo cp -vf $(iso) /var/lib/tftpboot/revonsos/
