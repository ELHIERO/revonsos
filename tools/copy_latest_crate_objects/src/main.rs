...
//! it is *not* valid to have multiple different versions of a first-party RevonsOs crate object file
...
//! 
//! Thus, this application selects the latest version of each first-party RevonsOs crate, 
...
fn main() -> Result<(), String> {
    ...
    opts.reqopt(
        ...
        "(required) path to the input directory of compiled crate .rlib, .rmeta, and .o files, 
         typically the `target`, e.g., \"/path/to/RevonsOS/target/$TARGET/release/deps/\"", 
        "INPUT_DIR"
    );
    opts.reqopt(
        "", 
    ...
    // that we ask cargo to build for us for RevonsOs's custom platform target
