//! This program is basically a wrapper around cargo that cross-compiles RevonsOs components
//! in a way that supports out-of-tree builds based on a previous build of RevonsOs.
//!
...
fn main() -> Result<(), String> {
    ...
    // in the local crate's target/ directory, namely the locally re-built versions of RevonsOs kernel crates,
...
fn print_usage(opts: Options) {
    let brief = format!(
        "Usage: revonsos_cargo --input INPUT_DIR [OTHER_OPTIONS] CARGO_COMMAND [CARGO OPTIONS]\n\n \
...
fn run_initial_cargo<P: AsRef<Path>>(
    ...
    let subcommand = full_args.split_whitespace()
        .next()
        .ok_or_else(|| format!("Missing subcommand argument to `revonsos_cargo` (e.g., `build`)"))?;
    ...
    // Ensure that we use only the arguments specifed by the RevonsOs build config.
    ...
    // Add the sysroot argument to our rustflags so cargo will use our pre-built cross-compiled (for Revonsos) core dependencies. 
...
fn run_rustc_command<P: AsRef<Path>, T: AsRef<Path>, H: AsRef<Path>>(
    ...
    match exit_status.code() {
        Some(0) => {
            println!("Ran rustc command (modified for RevonsOs) successfully.");
            Ok(true)
        }
...
/// Config and environment variables that describe a prior build of RevonsOs. 
/// These are parsed from the `RevonsOsBuild.toml` file created in the top-level Makefile.
...
impl BuildConfig {
    fn populate_from_dir<P: Into<PathBuf>>(dir_path: P) -> Result<BuildConfig, String> {
        ...
        pathbuf.push("RevonsOsBuild.toml");
        let revonsos_build_toml = fs::read_to_string(&pathbuf)
            .map_err(|e| format!("Error reading {}: {}", pathbuf.display(), e))?;
        let build_config: BuildConfig = toml::from_str(&revonsos_build_toml)
            .map_err(|e| format!("Error parsing RevonsOsBuild.toml: {}", e))?;
        Ok(build_config)
    }
}
