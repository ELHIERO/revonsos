...
//! options should be enabled when building revonsos.  
...
fn main() {
    println!("cargo:rerun-if-env-changed=REVONSOS_CONFIG");
    let configs = std::env::var("REVONSOS_CONFIG").unwrap_or(String::new()).to_lowercase();
    ...
    // we must manually add the ones used in RevonsOs.
