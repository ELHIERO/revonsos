...
### This script is invoked from the RevonsOs top-level Makefile, using 'make build_server',
### should be run on a machine that you can host a publicly-accessible HTTP server.


### the directory containing this script 
SCRIPTS_DIR="$(dirname "$(readlink -f "$0")")"
REVONSOS_BASE_DIR=$SCRIPTS_DIR/..
TOOLS_DIR=$REVONSOS_BASE_DIR/tools
...
if [ -z $HTTP_ROOT ] ; then 
  HTTP_ROOT=$REVONSOS_BASE_DIR/.revonsos_build_server
