...
### This script builds and installs GCC and binutils that are cross-compiled to target RevonsOs on x86_64. 
...
### so let's use it to build a new version of gcc that cross-compiles for the RevonsOs target.
mkdir -p $DEST/cross
export PREFIX="$DEST/cross"
export TARGET=x86_64-elf
export PATH="$PREFIX/bin:$PATH"

### Build a cross-compiled version of binutils that targets RevonsOs.
cd $SRC
