...
### This script sets up the build environment of tools required for building RevonsOs on macOS.
...
#brew untap revonsos-os/gcc_cross_compilers || true
#brew tap revonsos-os/gcc_cross_compilers
...
export SRC_DIR="$HOME/revonsos_tools_src"
export PREFIX="$HOME/revonsos_tools_opt/"
...
if [ ! -d "grub" ]; then
  ...
  echo ""
  git clone --depth 1 https://github.com/revonsos-os/grub.git
  cd grub
  sh autogen.sh
  mkdir -p build-grub
  cd build-grub
