//! # Overview of RevonsOs
//! 
//! RevonsOs is a new OS written from scratch in Rust, with primary goals of runtime composability and state spill freedom.
//! 
//! The RevonsOs kernel is composed of many small entities, each contained within a single Rust crate, and built all together as a cargo virtual workspace. 
//! All crates in RevonsOs are listed in the sidebar to the left. Click on a crate name to read more about it, and the functions and types it provides.
...
//! 
//! For a more general description of RevonsOs's concepts, organization, build process, contributing guidelines, and more,
//! please see the revons website
//! 
//! ## Basic Overview of All Crates
//! 
//! One-line summaries of what each crate includes (may be incomplete/out-of-date):
//! 
//! * `acpi`: ACPI (Advanced Configuration and Power Interface) support for RevonsOs, including multicore discovery.
//! * `apic`: APIC (Advanced Programmable Interrupt Controller) support for RevonsOs (x86 only), including apic/xapic and x2apic.
...
//! * `captain`: The main driver of RevonsOs. Controls the loading and initialization of all subsystems and other crates.
...
//! * `gdt`: GDT (Global Descriptor Table) support (x86 only) for RevonsOs.
//! * `interrupts`: Interrupt configuration and handlers for RevonsOs. 
//! * `ioapic`: IOAPIC (I/O Advanced Programmable Interrupt Controller) support (x86 only) for RevonsOs.
...
//! * `pci`: Basic PCI support for RevonsOs, x86 only.
//! * `pic`: PIC (Programmable Interrupt Controller), support for a legacy interrupt controller that isn't used much.
//! * `pit_clock`: PIT (Programmable Interval Timer) support for RevonsOs, x86 only.
...
//! * `tss`: TSS (Task State Segment support (x86 only) for RevonsOs.
